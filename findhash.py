...
def find_hash(o, hash_functions, hash_size):
    ...
    for f in hash_functions:
        ...
        if ((hash_size < best_result['size']) or
            ((hash_size == best_result['size']) and (num_collisions < best_result['collisions']))):
                best_size_result = { 'size' : hash_size, 'collisions' : num_collisions, 'f_hash_size' : f_hash_size, 'f': f } 
        if ((result['collisions'] < best_result['collisions']) or
            ((result['collisions'] == best_result['collisions']) and (hash_size < best_result['size']))):
